...
export default function Sidebar({ onClose }: { onClose?: () => void }) {
  ...
  return (
    <aside className="flex h-screen w-60 flex-col bg-[hsl(var(--card))] border-r border-[hsl(var(--border))]">
      ...
      {/* Footer card */}
      <div className="mx-3 mb-3 rounded-lg bg-gradient-to-br from-[hsl(var(--primary))] to-[hsl(var(--primary)/0.8)] p-3 text-white">
        <p className="text-[11px] font-semibold">Pro Plan Active</p>
        <p className="mt-0.5 text-[10px] text-white/70">Full analytics access</p>
        <div className="mt-2 h-1 overflow-hidden rounded-full bg-white/20">
          <div className="h-full w-[72%] rounded-full bg-white/80 transition-all duration-500" />
        </div>
        <p className="mt-1 text-[9px] text-white/60">72% of monthly quota used</p>
      </div>
    </aside>
  );
}
