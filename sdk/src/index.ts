...
// ── Screenshot capture removed - will be replaced with rrweb ─────────────
...
export async function init(userConfig: EmoraTestConfig): Promise<void> {
  ...
  // Start rrweb recording (fails silently if rrweb doesn't load)
  if (session.sampledIn) {
    try {
      rrwebRecorder = new RRWebRecorder();
      rrwebRecorder.start();
      if (config.debug) {
        console.debug("[EmoraTest] rrweb recording started");
      }
    } catch (err) {
      if (config.debug) {
        console.warn("[EmoraTest] rrweb recording failed to start (continuing without replay):", err);
      }
      rrwebRecorder = null;
    }
  }
  ...
  const unloadHandler = () => {
    ...
    // Send rrweb events if recording
    if (rrwebRecorder && rrwebRecorder.isRecording() && sessionManager) {
      const sessionId = sessionManager.getSessionId();
      if (sessionId) {
        const recording = rrwebRecorder.stop();
        if (recording && transport) {
          transport.sendBeaconRRWeb(sessionId, {
            events: recording.events,
            duration_ms: recording.durationMs,
            events_count: recording.eventsCount,
          });
        }
      }
    }
...
export async function destroy(): Promise<void> {
  ...
  // Stop rrweb recording and send events
  if (rrwebRecorder && rrwebRecorder.isRecording() && sessionManager && transport) {
    const sessionId = sessionManager.getSessionId();
    if (sessionId) {
      const recording = rrwebRecorder.stop();
      if (recording) {
        try {
          await transport.sendRRWebEvents(sessionId, {
            events: recording.events,
            duration_ms: recording.durationMs,
            events_count: recording.eventsCount,
          });
          ...
          if (config?.debug) {
            console.warn("[EmoraTest] Failed to send rrweb events:", err);
          }
        }
      }
    }
